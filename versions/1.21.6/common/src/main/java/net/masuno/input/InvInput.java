...
public final class InvInput {
	public static final String CATEGORY = "key.category.colored_slots.inventory";
	...
	public static void tick(Minecraft client, boolean editingScreen) {
		...
		if (editingScreen) {
			...
			if (InputConstants.isKeyDown(window, TOGGLE_SLOT_EDITING.getDefaultKey().getValue())) {
			...
			Sneaking = InputConstants.isKeyDown(window, client.options.keyShift.getDefaultKey().getValue());
